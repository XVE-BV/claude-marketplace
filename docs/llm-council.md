# LLM Council

When you're stuck on a real decision, the LLM Council gives you five independent perspectives at once — then pressure-tests them against each other — before delivering a clear recommendation.

## When to use it

Say any of these to Claude:

> "Council this"  
> "War room this"  
> "Pressure-test this"  
> "Stress-test this"  
> "I can't decide between X and Y"  
> "Validate this for me"

**Good fit:** decisions with real stakes and no obvious right answer — pricing, hiring, build vs buy, strategy pivots, major investments.

**Not for:** looking something up, writing something, or decisions you've already made.

## What happens

### 1 — Claude frames the question

Claude reads your context (workspace, memory, anything you've shared) and writes a clear version of the decision you're facing. If it's too vague, it asks one clarifying question.

### 2 — Five advisors weigh in simultaneously

Five AI advisors each respond independently — they can't see each other's answers. Each has a distinct lens:

| Advisor | What they look for |
|---------|-------------------|
| **The Contrarian** | What's wrong, what's missing, what will fail |
| **The First Principles Thinker** | Whether you're even asking the right question |
| **The Expansionist** | Upside and opportunity everyone else is missing |
| **The Outsider** | What looks confusing or wrong to someone with zero context |
| **The Executor** | Whether this can actually be done, and what the first step is |

### 3 — Peer review

The five responses are anonymised (A through E) and handed to five reviewers. Each reviewer identifies: which response was strongest, which had the biggest blind spot, and what all five missed.

This is what separates the Council from a single AI answer — the weak spots get called out.

### 4 — The Chairman delivers a verdict

One final agent reads everything: all five advisor responses, all five peer reviews. It produces:

- **Where everyone agrees** — high-confidence signals
- **Where they clashed** — genuine disagreements and why
- **What nobody caught** — blind spots that only emerged through review
- **The recommendation** — a direct answer, not "it depends"
- **The one thing to do first** — a single concrete next step

The Chairman can disagree with the majority if the reasoning supports it.

### 5 — You get a report

Claude saves an HTML report (opens automatically) and a full transcript. The report shows the verdict prominently, with all advisor responses and reviews available to expand if you want the detail.

## Example

> "Should I launch a $97 workshop or a $497 course first?"

The Council would surface: cash flow differences, audience trust signals, delivery risk, what competitors are missing, and whether the question is really about revenue or validation — then give you a direct answer and a first step.
